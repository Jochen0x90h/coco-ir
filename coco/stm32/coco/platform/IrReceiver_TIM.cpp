...
IrReceiver_TIM::IrReceiver_TIM(Loop_Queue &loop, gpio::Config inputPin, int inputIndex, const timer::GpInfo &timerInfo, Hertz<> clock)
    ...
    , loop(loop)
{
    ...
    // configure timer
    this->timer = timerInfo.configure()
        .setCountFrequency(clock, 20kHz) // 50μs resolution
        ...
        .setInput1(timer::InputMode::EDGE_BOTH | (inputIndex == 1 ? timer::InputMode::INPUT_DEFAULT : timer::InputMode::INPUT_ALTERNATE) | timer::InputMode::CAPTURE)
        .setInput2(timer::InputMode::EDGE_BOTH | timer::InputMode::INPUT_DEFAULT);
    this->timerIrq = timerInfo.irq;
